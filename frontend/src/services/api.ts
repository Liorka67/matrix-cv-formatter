...
export class ApiService {
  ...
  static async downloadDocx(cv: MatrixCV, language: 'he' | 'en'): Promise<void> {
    const response = await api.post('/api/download/docx', { cv, language }, { responseType: 'blob' });
  ...
  static async downloadPdf(cv: MatrixCV, language: 'he' | 'en'): Promise<void> {
    const response = await api.post('/api/download/pdf', { cv, language }, { responseType: 'blob' });
    const url = URL.createObjectURL(new Blob([response.data]));
    const a = document.createElement('a');
    a.href = url;
